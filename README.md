...
[![PyPI Version](https://img.shields.io/badge/pypi-v2.2.0-blue)](https://pypi.org/project/agentoptim/)
...
Version 2.2.0 adds a powerful system message optimization tool that automatically generates, evaluates, and ranks system messages for optimal performance:
...
1. **Core System Message Generator** 🔄
   ...
   - [x] **1.2** Create system message generation pipeline using meta-prompt
   - [x] **1.3** Implement diversity controls for candidate generation
...
2. **Self-Optimization Mechanism** 🔄
   ...
   - [x] **2.4** Add automatic optimization trigger based on performance thresholds
   - [x] **2.5** Implement domain-specific generator specialization
   - [x] **2.6** Add manual trigger for forced self-optimization

3. **Evaluation Integration** 🔄
   - [x] **3.1** Connect to existing evaluation system using EvalSets
   - [x] **3.2** Implement parallel processing for candidate evaluation
   - [x] **3.3** Create detailed scoring and ranking algorithm
   - [x] **3.4** Add per-criterion performance breakdown
   - [x] **3.5** Implement weighted scoring based on criteria importance
   - [x] **3.6** Develop confidence metrics for optimization quality
...
- [x] **Example Library**
  ...
  - [x] [system_message_optimization.py](examples/system_message_optimization.py) - Optimizing system messages (v2.2.0)

## 💻 Quick Example

```python
import asyncio
from agentoptim import manage_evalset_tool, manage_eval_runs_tool, optimize_system_messages_tool
...
async def main():
    ...
    # 6️⃣ NEW in v2.2.0: Optimize system messages for a user query
    user_query = "How do I reset my password?"
    optimization = await optimize_system_messages_tool(
        action="optimize",
    ...
    # Show the best system message
    best_system_message = optimization["candidates"][0]["system_message"]
    print(f"\nOptimized system message:")
    print(best_system_message)
    
    # View performance breakdown
    performance = optimization["candidates"][0]["performance"]
...
Optimized system message:
You are a customer support assistant specializing in account security and access management. Your goal is to provide clear, step-by-step instructions for account-related issues. When users need help with password resets or authentication setup, offer concise guidance with specific UI element names and exact steps. Prioritize security best practices while being accessible to users with varying technical expertise. Avoid unnecessary information and focus on providing direct, actionable solutions to the user's specific problem.
...
> **Note:** In version 2.2.0, we've added powerful system message optimization capabilities. 
